...
def get_data(filename):
	...
	healthy = df_type[df_type['!Sample_title'].str.contains('healthy|normal')
		 == True]
	cancer_patients = df_type[df_type['!Sample_title'].str.contains('healthy|normal')
		 != True]
...
def visualize_data(X, df, df_type):
	...
	print dv.head()
	featuresForGraph = dv.loc['Probe',:]
	featuresForGraph = featuresForGraph[0:8]
	plt.figure()
	df = df.loc[:, featuresForGraph]
