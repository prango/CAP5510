...
def visual_compare_methods():
	max_features = 500
	datasets = np.array(['GSE32474', 'GSE19804', 'GSE27562', 'GSE59856', 'GSE33315'])
	counter = 0
	method_combo = []
	
	feature_selection = ['Semi-supervised variance by mean ratio',
	...
	for model in classifiers:
		for feature_method in feature_selection:
			...
			for dataset in datasets:
				filename = '/Users/GodSpeed/Documents/Courses/Bioinformatics/Project/Datasets/' + dataset + '_series_matrix.txt'
				...
				for num_of_dims in range(100, max_features + 1, 100):
					...
					for iter in range(0,5):
						X = Xtemp
						if feature_method == 'Semi-supervised variance by mean ratio':
			...
			f_measure_dims = (pd.DataFrame(data = f_measure_each, 
				...
				index = datasets)).transpose()
			
			print f_measure_dims
			
			#plt.figure()
			f_measure_dims.plot()
			plt.title("Accuracy rate for " + feature_method + ' and ' + model)
			plt.ylabel('Accuracy %')
			plt.xlabel('Number of features after feature selection method')
			plt.show()
	#return performance, feature_selection, classifiers, f_measure_dims

def main():
	warnings.filterwarnings("ignore")
	visual_compare_methods()
